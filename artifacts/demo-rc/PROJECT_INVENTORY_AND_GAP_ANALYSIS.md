...
1. **Fase 7E — Demo Release Candidate (Completar)**
   - Objetivo: Proveer runbooks, scripts operativos (`demo:doctor`) y UI de showcase.
2. **Fase 7F — Internal UAT**
   - Objetivo: Prueba en vivo interna con MercadoPago Sandbox conectado (inyección de claves reales temporales).
3. **Fase 8A — Foundry/Anvil local**
   - Objetivo: Sincronización Web3 local (Ledger SQL vs Smart Contract).
4. **Fase 8B/C — Cloud Deploy (Staging Demo)**
   - Objetivo: Migración de infraestructura hacia NEXUS AWS/GCP o Vercel.
