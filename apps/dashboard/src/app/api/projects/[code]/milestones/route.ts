import { NextRequest, NextResponse } from "next/server";
import { db, core } from "@/server/db";
import { eq } from "drizzle-orm";
import { getSessionUser, isAdmin } from "@/lib/auth/session";
import { loadProjectByCode } from "@/lib/projects/load";

export const dynamic = "force-dynamic";

export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ code: string }> }
) {
  try {
    const session = await getSessionUser();
    if (session && !isAdmin(session.role)) {
      return NextResponse.json({ error: "Solo admin" }, { status: 403 });
    }
    const { code } = await ctx.params;
    const project = await loadProjectByCode(code);
    if (!project) return NextResponse.json({ error: "Proyecto no encontrado" }, { status: 404 });
    const body = await req.json();
    const title = String(body.title || "").trim();
    if (!title) return NextResponse.json({ error: "title obligatorio" }, { status: 400 });

    const [milestone] = await db
      .insert(core.projectMilestones)
      .values({
        projectId: project.id,
        title,
        description: body.description ? String(body.description) : null,
      })
      .returning();

    await db.insert(core.auditEvents).values({
      actorId: session?.id,
      action: "milestone.create",
      entityType: "milestone",
      entityId: milestone.id,
      payload: { project: code, title },
    });

    return NextResponse.json({ milestone }, { status: 201 });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Error";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}

export async function PATCH(
  req: NextRequest,
  ctx: { params: Promise<{ code: string }> }
) {
  try {
    const session = await getSessionUser();
    if (session && !isAdmin(session.role)) {
      return NextResponse.json({ error: "Solo admin" }, { status: 403 });
    }
    await ctx.params;
    const body = await req.json();
    if (!body.id) return NextResponse.json({ error: "id obligatorio" }, { status: 400 });

    const patch: Record<string, unknown> = {};
    if (body.status) {
      patch.status = body.status;
      if (body.status === "done") patch.completedAt = new Date();
    }
    if (body.evidenceUrl !== undefined) patch.evidenceUrl = body.evidenceUrl;

    const [milestone] = await db
      .update(core.projectMilestones)
      .set(patch)
      .where(eq(core.projectMilestones.id, body.id))
      .returning();

    await db.insert(core.auditEvents).values({
      actorId: session?.id,
      action: "milestone.update",
      entityType: "milestone",
      entityId: body.id,
      payload: body,
    });

    return NextResponse.json({ milestone });
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Error";
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
