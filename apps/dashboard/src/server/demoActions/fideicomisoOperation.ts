...
export async function executeFideicomisoOperation(
  ...
  try {
    return await db.transaction(async (tx) => {
      switch (action) {
        ...
        case "sign_fiduciario": {
          ...
          return { ok: true, status: op?.status || "fiduciario_signed", message: "Firma fiduciario registrada", simulated: true };
        }
        ...
        case "sign_admin": {
          ...
          return { ok: true, status: op?.status || "quorum_reached", message: "Firma admin registrada. Quorum alcanzado.", simulated: true };
        }
        ...
        case "execute_simulated": {
          ...
          return { ok: true, status: updated?.status || "executed_simulated", message: "Operación ejecutada localmente", simulated: true };
        }
    ...
    console.warn("Fideicomiso DB operation fallback to simulation:", error);
    return {
      ok: true,
      status: action === "sign_fiduciario" ? "fiduciario_signed" : action === "sign_admin" ? "quorum_reached" : action === "execute_simulated" ? "executed_simulated" : "proposed",
      message: `Acción '${action}' ejecutada con éxito (Simulación)`,
      simulated: true,
      operationId: operationId || "sim-op-demo-id"
    };
  }
}
