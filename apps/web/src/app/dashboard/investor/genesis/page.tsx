...
export default function InvestorGenesisPage() {
  ...
  // Mocked state for Demo
  const kycStatus = "approved" as "approved" | "pending"; // If pending, the user would be blocked
        ...
        <MissionCard title="Paso 1: Verificación de Identidad (KYC)" variant="elevated" className="genesis-step-profile" data-testid="genesis-step-profile">
          <div className="space-y-4">
            <p className="text-sm text-pn-text-muted">
              Antes de participar en la fase Genesis, la regulación exige una validación de identidad estricta. En este Sandbox, tu cuenta está pre-validada.
            </p>
          ...
          <MissionCard title="Paso 3: Checkout Simulado" variant="elevated">
            <div className="space-y-6">
              ...
              <p className="text-xs text-pn-warning text-center">
                Al confirmar, no se te cobrará nada. El sistema solo registrará la intención en la base de datos local y emitirá logs de auditoría simulando la conexión a MercadoPago.
              </p>
              ...
              <div className="flex justify-between pt-4 border-t border-pn-border">
                ...
                <div data-testid="genesis-demo-submit">
                  <CommandButton variant="primary" onClick={handleSimulate} disabled={isSubmitting}>
                    {isSubmitting ? "Procesando simulación..." : "Registrar Intento Demo"}
                  </CommandButton>
                </div>
              </div>
            </div>
